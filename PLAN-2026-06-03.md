...
> 最后更新: 2026-06-03 (数据库迁移完成，API 验证通过)
...
**数据库迁移 (2026-06-03)**:
- [x] `monsters` 表新增 `narrative_tags_json` 列 — ALTER TABLE 已执行
- [x] 4 个怪物叙事标签已更新 (station_rat/broken_avatar_shadow/station_keeper_fragment/mall_hunger_echo)
- [x] 20 个 ORV 称号已播种 — titles 表从 6 增至 26
- [x] 端到端验证通过：
  - `/api/titles/all` → 返回 26 个称号（含 narrative 阵营标签）
  - `/api/combat/monsters` → 返回 4 个怪物（含 narrativeTags）
  - `/api/explore/start` → 探索流程正常，称号自动解锁
  - `/api/combat/start` → 战斗正常运行，位格/叙事压制逻辑在服务端计算
