    ...
    return items.map(item => `
      <div class="drawer-card">
        <div class="drawer-card-title">
          ...
          <span class="fs-11 text-secondary ml-8">x${item.quantity}</span>
        </div>
        ...
        <div class="drawer-card-actions">
          <span class="fs-12 text-secondary">售价: ${item.sell_price || 0}</span>
    ...
    slotOrder.forEach(function(slot) {
      ...
      if (eq) {
        html += '<div class="equip-slot equipped">' +
          ...
          '<button class="btn-action btn-sm btn-cancel mt-4" onclick="GameClient.unequipItem(\'' + slot + '\')">卸下</button>' +
          '</div>';
      } else {
