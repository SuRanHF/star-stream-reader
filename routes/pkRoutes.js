...
router.post('/challenge', (req, res) => {
  try {
    ...
    try {
      ...
      if (e.code === 'PLAYER_RESTING') return res.status(400).json({ code: 'PLAYER_RESTING', message: e.message });
      throw e;
    }
...
router.post('/challenge/resolve', (req, res) => {
  try {
    ...
    if (!challengeId || !playerId) return res.status(400).json({ code: 'MISSING_PARAMS', message: '缺少必要参数' });
