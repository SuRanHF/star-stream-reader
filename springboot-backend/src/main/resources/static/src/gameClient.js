...
const GameClient = {
  ...
  async handleLogin(usernameOrEmail, password) {
    ...
    try {
      ...
      if (result.success) {
        // Spring Boot AuthResult: { id, username, email, role, token }
        Storage.setSession(result.data.token, result.data.username);
        this._currentUser = { id: result.data.id, username: result.data.username, email: result.data.email, role: result.data.role };
  ...
  async createPlayerBound(name) {
    ...
    try {
      ...
      // Spring Boot returns Result<PlayerVO> — data IS the player, no wrapper
      if (result.success && result.data) {
  ...
  async fetchChapter(opts) {
    opts = opts || {};
    try {
      const result = await API.getCurrentStory(this.playerId);
      const data = result.data || result;

      if (result.error || !result.success) {
        UI.showError((result.error && result.error.message) || '加载章节失败');
        return;
      }
      ...
      if (this._initialLogsLoaded === undefined && data.player && data.player.logs && data.player.logs.length > 0) {
      ...
      try {
        const titlesResult = await API.getTitles(this.playerId);
        const titles = titlesResult.data || titlesResult;
        const titleCount = document.getElementById('titleCount');
        if (titleCount) titleCount.textContent = `称号 ${((titles && titles.titles) || (titles && titles.length) ? (titles.titles || titles) : []).length}`;
      } catch (e) { /* ignore */ }
  ...
  async doExplore(locationKey) {
    ...
    try {
      ...
      const apiResult = await API.startExploration(this.playerId, locationKey, firstExplore);
      const result = apiResult.data || apiResult;

      // Check for error
      if (!result || apiResult.error || (apiResult.success === false)) {
        const msg = (apiResult.error && apiResult.error.message) || '探索失败';
      ...
      // Log rewards
      if (result.rewards && Object.keys(result.rewards).length > 0) {
        ...
        if (rew.equipment) rewardParts.push('装备: ' + (rew.equipment_name || (typeof rew.equipment === 'string' ? rew.equipment : '')));
  ...
  _startRecoveryLoop() {
    ...
    this._recoveryInterval = setInterval(async () => {
      ...
      try {
        const playerResult = await API.getPlayer(this.playerId); const player = (playerResult && playerResult.data) || playerResult || {};
        UI.renderLeftPanel(player);
        this.updateMainActionBar(player);
        this._isResting = !!(player.stats && player.stats.isResting);
  ...
  async loadInventory() {
    try {
      const invResult = await API.getInventory(this.playerId);
      const items = (invResult && invResult.data) || invResult || [];
      var recipes = [];
      try {
        var recipesResp = await API.getSynthesisRecipes();
        var recipesData = (recipesResp && recipesResp.data) || recipesResp || {};
        recipes = recipesData.recipes || recipesData || [];
      } catch (e) { /* recipes not critical */ }
      // Compute available counts per recipe
      var itemMap = {};
      (items || []).forEach(function(it) { itemMap[it.item_key || it.itemKey] = it.quantity; });
      ...
      const playerResult = await API.getPlayer(this.playerId);
      const player = (playerResult && playerResult.data) ? playerResult.data : (playerResult || {});
      UI.renderLeftPanel(player);
    } catch (e) {
  ...
  async loadEquipment() {
    try {
      var equipResult = await API.getEquipment(this.playerId);
      // Unwrap Result<T>: could be { success, data: [...] } or direct array
      var allEquipment = (equipResult && equipResult.data) || equipResult || [];
      var equipped = (allEquipment || []).filter(function(e) { return e.equipped; });
      var available = (allEquipment || []).filter(function(e) { return !e.equipped; });
      // Normalize field names: Java camelCase → frontend snake_case
      allEquipment.forEach(function(e) {
        if (e.equipmentKey !== undefined && e.equipment_key === undefined) e.equipment_key = e.equipmentKey;
        if (e.maxDurability !== undefined && e.max_durability === undefined) e.max_durability = e.maxDurability;
        if (e.sellPrice !== undefined && e.sell_price === undefined) e.sell_price = e.sellPrice;
        if (e.baseStats !== undefined) e.stats = e.baseStats;
        if (e.requiredLevel !== undefined && e.required_level === undefined) e.required_level = e.requiredLevel;
        if (e.setKey !== undefined && e.set_key === undefined) e.set_key = e.setKey;
        if (e.canEquip !== undefined && e.can_equip === undefined) e.can_equip = e.canEquip;
        if (e.owned === undefined) e.owned = true;
      });
      var activeSets = [];
      try {
        var setsResult = await API.getEquipmentSets();
        var setsData = (setsResult && setsResult.data) || setsResult || [];
        activeSets = setsData.active_sets || setsData || [];
      } catch (e) { /* sets not critical */ }
      var contentHTML = UI.renderEquipment(equipped, available, activeSets);
      UI.openDrawer('装备', contentHTML);
      var playerResult = await API.getPlayer(this.playerId);
      var player = (playerResult && playerResult.data) ? playerResult.data : (playerResult || {});
      UI.renderLeftPanel(player);
  ...
  async loadSkills() {
    try {
      ...
      var skillsData = (skillsResp && skillsResp.data) || skillsResp || [];
      var skills = skillsData.skills || skillsData || [];
      // Normalize field names: Java camelCase → frontend snake_case
      if (!Array.isArray(skills)) skills = [];
      skills.forEach(function(sk) {
        if (sk.skillKey !== undefined && sk.skill_key === undefined) sk.skill_key = sk.skillKey;
        if (sk.type !== undefined && sk.skill_type === undefined) sk.skill_type = sk.type;
        if (sk.cooldownSeconds !== undefined && sk.cooldown === undefined) sk.cooldown = sk.cooldownSeconds;
        if (sk.requiredFragments !== undefined && sk.required_fragments === undefined) sk.required_fragments = sk.requiredFragments;
        if (sk.unlocked === undefined) sk.unlocked = true; // in player skills list = unlocked
        if (sk.can_unlock === undefined) sk.can_unlock = false;
      });
      ...
      try {
        ...
        var fsData = (fsResp && fsResp.data && fsResp.data.skills) ? fsResp.data.skills : [];
        factionSkills = fsData || [];
  ...
  async loadTitles() {
    try {
      const titlesResult = await API.getTitles(this.playerId);
      const titlesData = (titlesResult && titlesResult.data) || titlesResult || {};
      const titles = titlesData.titles || titlesData || [];
  ...
  async loadPK() {
    try {
      ...
      const oppUnwrapped = (oppData && oppData.data) || oppData || {};
      const recUnwrapped = (recData && recData.data) || recData || {};
      const opponents = oppUnwrapped.opponents || oppUnwrapped || [];
      const records = recUnwrapped.records || recUnwrapped || [];
      const opponentsHTML = UI.renderPKOpponents(opponents);
      const recordsHTML = UI.renderPKRecords(records);
  ...
  async doPKResolve(challengeId, accept) {
    try {
      ...
      if (accept && result.data && result.data.battle) {
        ...
        if (resp && resp.data) {
          UI.renderLeftPanel(resp.data);
          this._currentPlayer = resp.data;
        }
      } else if (!accept) {
  ...
  async loadRankings() {
    ...
    try {
      ...
      if (tab === 'hub') {
        ...
        var avatarResult = await API.getAvatarRankLeaderboard(50);
        var avatarData = (avatarResult && avatarResult.data) || avatarResult || {};
        contentHTML += UI.renderAvatarRankLeaderboard(avatarData.rankings || avatarData || []);
      } else if (tab === 'broadcast') {
        var bdata = await API.getBroadcastLeaderboard(50);
        var bRankings = (bdata && bdata.data) || bdata || [];
        contentHTML += UI.renderBroadcastLeaderboard(bRankings);
      } else {
        var pkResult = await API.getRankings(50);
        var pkdata = (pkResult && pkResult.data) || pkResult || {};
        contentHTML += UI.renderRankings(pkdata.rankings || pkdata || []);
      }
  ...
  async _renderRankingsHub() {
    ...
    try {
      ...
      var pkResult = await API.getRankings(5);
      var pkData = (pkResult && pkResult.data) || pkResult || {};
      var pkRankings = pkData.rankings || pkData || [];
      var avatarResult = await API.getAvatarRankLeaderboard(5);
      var avatarData = (avatarResult && avatarResult.data) || avatarResult || {};
      var avatarRankings = avatarData.rankings || avatarData || [];
      var broadcastResult = await API.getBroadcastLeaderboard(5);
      var broadcastData = (broadcastResult && broadcastResult.data) || broadcastResult || [];

      html += '<div class="drawer-section-label">PK 排行榜 (Top 5)</div>';
      if (pkRankings.length > 0) {
        html += '<table class="drawer-table mb-16"><thead><tr><th>#</th><th>玩家</th><th>Lv</th><th>评分</th></tr></thead><tbody>';
        for (var i = 0; i < pkRankings.length; i++) {
          var r = pkRankings[i];
          html += '<tr><td class="' + (i < 3 ? 'rank-top' : '') + '">' + (r.rank || i + 1) + '</td><td>' + (r.player_name || r.playerName) + '</td><td>' + (r.level || '') + '</td><td>' + (r.rating || '') + '</td></tr>';
        }
        html += '</tbody></table>';
      } else {
      ...
      if (avatarRankings.length > 0) {
        html += '<table class="drawer-table mb-16"><thead><tr><th>#</th><th>玩家</th><th>化身位阶</th><th>Lv</th></tr></thead><tbody>';
        for (var j = 0; j < avatarRankings.length; j++) {
          var ar = avatarRankings[j];
          html += '<tr><td class="' + (j < 3 ? 'rank-top' : '') + '">' + (ar.rank || j + 1) + '</td><td>' + (ar.player_name || ar.playerName) + '</td><td class="text-gold">' + (ar.avatarRank || '') + '级·' + (ar.avatarRankName || '') + '</td><td>' + (ar.level || '') + '</td></tr>';
        }
        html += '</tbody></table>';
      } else {
        html += '<p class="text-secondary text-center px-12 py-8">暂无数据</p>';
      ...
      if (broadcastData.length > 0) {
        ...
        for (var k = 0; k < broadcastData.length; k++) {
          var br = broadcastData[k];
          html += '<tr><td class="' + (k < 3 ? 'rank-top' : '') + '">' + (br.rank || k + 1) + '</td><td>' + (br.player_name || br.playerName) + '</td><td>' + (br.level || '') + '</td><td class="text-teal">' + (br.total_contribution || br.totalContribution || '') + '</td></tr>';
        }
        html += '</tbody></table>';
      } else {
        html += '<p class="text-secondary text-center px-12 py-8">暂无数据</p>';
  ...
  async loadBroadcastProgress(eventId) {
    try {
      const progressResp = await API.getBroadcastProgress(eventId);
      const progress = (progressResp && progressResp.data) || progressResp || {};
      UI.renderBroadcastProgress(progress);
      const rankingResp = await API.getBroadcastRanking(eventId, 10);
      const ranking = (rankingResp && rankingResp.data) || rankingResp || [];
  ...
  async loadMyContribution(eventId, playerId) {
    try {
      const resp = await API.getMyContribution(eventId, playerId);
  ...
  async openFaction() {
    ...
    try {
      var factionsResult = await API.getFactions();
      var factions = (factionsResult && factionsResult.data) || factionsResult || [];
      var myFaction = null;
      try {
        var myResult = await API.getMyFaction(this.playerId);
        myFaction = (myResult && myResult.data) || myResult || null;
      } catch (e) { /* may not have faction yet */ }
      var weeklyWar = null;
      try {
        var warResult = await API.getWeeklyWar();
        var wars = (warResult && warResult.data) || warResult || [];
        weeklyWar = (wars && wars.length > 0) ? wars[0] : null;
      } catch (e) { /* non-critical */ }
      var content = document.getElementById('factionContent');
      if (content) {
        content.innerHTML = UI.renderFactionPanel(this.playerId, myFaction, factions, weeklyWar);
      }
    } catch (e) {
      var content = document.getElementById('factionContent');
  ...
  // ===== Trade (交易市场) =====
  async openTrade() {
    UI.openDrawer('交易市场', '<div id="tradeContent"><div class="loading-placeholder"><div class="loading-spinner"></div></div></div>');
    try {
      var listingsResult = await API.getActiveListings();
      var listings = (listingsResult && listingsResult.data) || listingsResult || [];
      var myListings = [];
      if (this.playerId) {
        try {
          var myResult = await API.getMyListings(this.playerId);
          myListings = (myResult && myResult.data) || myResult || [];
        } catch (e) { /* may not have listings */ }
      }
      var content = document.getElementById('tradeContent');
      if (content) {
        content.innerHTML = UI.renderTradePanel(this.playerId, listings, myListings);
      }
    } catch (e) {
      var content = document.getElementById('tradeContent');
      if (content) content.innerHTML = '<div class="error-state">加载失败: ' + UI.escapeHtml(e.message || e) + '</div>';
  ...
  async openParty() {
    ...
    try {
      var partiesResult = await API.getActiveParties();
      var parties = (partiesResult && partiesResult.data) || partiesResult || [];
      var myPartyRaw = this.playerId ? await API.getMyParty(this.playerId) : null;
      var myPartyData = (myPartyRaw && myPartyRaw.data) || myPartyRaw || null;
      // Normalize field names from Java to frontend expectations
      if (myPartyData) {
        if (myPartyData.leader_id !== undefined && myPartyData.leaderId === undefined) myPartyData.leaderId = myPartyData.leader_id;
        if (myPartyData.boss_key !== undefined && myPartyData.bossKey === undefined) myPartyData.bossKey = myPartyData.boss_key;
        if (myPartyData.leader_name !== undefined && myPartyData.leaderName === undefined) myPartyData.leaderName = myPartyData.leader_name;
        if (myPartyData.party_no !== undefined && myPartyData.partyNo === undefined) myPartyData.partyNo = myPartyData.party_no;
      }
      (parties || []).forEach(function(p) {
        if (p.leader_id !== undefined && p.leaderId === undefined) p.leaderId = p.leader_id;
        if (p.boss_key !== undefined && p.bossKey === undefined) p.bossKey = p.boss_key;
        if (p.leader_name !== undefined && p.leaderName === undefined) p.leaderName = p.leader_name;
      });
      var content = document.getElementById('partyContent');
      if (content) {
        content.innerHTML = UI.renderPartyPanel(this.playerId, myPartyData, parties || []);
      }
    } catch (e) {
      var content = document.getElementById('partyContent');
      if (content) content.innerHTML = '<div class="error-state">加载失败: ' + UI.escapeHtml(e.message || e) + '</div>';
  ...
  async openWorldBoss() {
    ...
    try {
      var statusResult = await API.getWorldBossStatus();
      var boss = (statusResult && statusResult.data) || statusResult || null;
      var ranking = [];
      if (boss && boss.bossNo) {
        try {
          var rankResult = await API.getWorldBossRanking(boss.bossNo);
          ranking = (rankResult && rankResult.data) || rankResult || [];
        } catch (e) { /* ranking non-critical */ }
      }
  ...
  async fightWorldBoss() {
    try {
      ...
      if (result && result.error) { UI.addLog(result.error.message, 'warning'); return; }
      if (data.damage) {
        UI.addLog((data.crit ? '暴击！' : '') + '对世界Boss造成 ' + data.damage + ' 点伤害', 'combat');
        if (data.defeated) {
  ...
  async openChat() {
    ...
    try {
      var resp = await API.getPlayer(this.playerId);
      var player = (resp && resp.data) ? resp.data : (resp || {});
  ...
  travelToLocation(locationKey) {
    ...
    API.getLocations(this.playerId).then(({ locations }) => {
      API.getPlayer(this.playerId).then(result => { const player = (result && result.data) || result || {};
        UI.showMapOverlay(locations, player);
      });
    }).catch(() => {});
  },
  ...
  async loadDetailedStats() {
    try {
      ...
      let globalWLS = 0;
      try {
        const wlResult = await API.getWorldlineStatus();
        const wlStatus = (wlResult && wlResult.data) || wlResult || {};
  ...
  async allocatePoints() {
    ...
    try {
      ...
      if (result.success) {
        ...
        UI.renderLeftPanel((result && result.data) || result || {});
  ...
  async openFactionSkills() {
    ...
    try {
      ...
      if (playerRes && playerRes.data) {
        constellationKey = (playerRes.data.stats && playerRes.data.stats.constellation) || '';
      }
